...
public static class DnsRecordBuilder
{
    ...
    private static byte[] SerializeCaa(CaaRecord caa)
    {
        ...
        // 1 byte: tag length
        // Variable length: tag (ASCII string)
        // Variable length: value (remaining bytes, NOT length-prefixed)
        var data = new List<byte>();
        ...
        // Value (NOT length-prefixed per RFC 6844 - occupies remaining RDATA bytes).
        // Only the tag is length-prefixed. CAA value is stored as a string in
        // DnsClient, but RFC allows binary; we encode it as UTF-8 bytes.
